...
func GetQueries() []string {
	return []string{
		...
		"CREATE TABLE IF NOT EXISTS article ( PRIMARY KEY (id), FOREIGN KEY (category)  REFERENCES category(name), FOREIGN KEY (subcategory)  REFERENCES subcategory(name), id INT AUTO_INCREMENT, name VARCHAR(255) NOT NULL, description VARCHAR(255) NOT NULL, price DECIMAL(11, 4) UNSIGNED NOT NULL,  image_name VARCHAR(255) NOT NULL, category VARCHAR(255) NOT NULL, subcategory VARCHAR(255) NOT NULL, in_stock INT UNSIGNED DEFAULT 0)",
		"CREATE TABLE IF NOT EXISTS article_has_rating ( PRIMARY KEY (id), FOREIGN KEY (article)  REFERENCES article(id)  ON DELETE CASCADE, id INT AUTO_INCREMENT, article INT NOT NULL, nr_up INT UNSIGNED DEFAULT 0, nr_down INT UNSIGNED DEFAULT 0)",
